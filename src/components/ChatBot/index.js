...
export default function ChatBot() {
  ...
  // Generate response - memoized
  const generateResponse = useCallback((userQuery) => {
  ...
  const handleSubmit = useCallback((e) => {
    ...
    setTimeout(() => {
      ...
      setMessages(prev => {
        ...
        setLastResponseIndex(newMessages.length - 1); // Set the index for the feedback
        return newMessages;
      });
      setIsLoading(false);
  ...
  const handleFeedback = useCallback((isHelpful) => {
  ...
  if (!isClient) {
    return (
      <div className={styles.chatbotContainer}>
        ...
        >
          <svg className={styles.chatIcon} viewBox="0 0 24 24" width="24" height="24" aria-hidden="true">
        ...
        {isOpen && (
          <div className={styles.chatWindow}>
            ...
            <div className={styles.chatMessages}>
                ...
                <div key={index}>
                  {message.isQuickQuestion ? (
                    <button 
                      ...
                      aria-label={`Ask question: ${message.text}`}
                    >
                      {message.text}
                    </button>
                  ) : (
              ...
              {isLoading && (
                <div className={`${styles.message} ${styles.botMessage} ${styles.loadingMessage}`} role="status" aria-label="Loading response">
